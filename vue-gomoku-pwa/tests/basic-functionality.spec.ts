...
test.describe('Gomoku PWA Basic Functions', () => {
  ...
  test('Page navigation', async ({ page }) => {
    ...
    await page.click('[href*="/settings"], button:has-text("Éè"), button:has-text("Settings")');
    await page.waitForURL('**/settings');
    await expect(page.locator('h1')).toBeVisible();
    
    // Test back button
    await page.click('text=·µ»Ø, text=Back, [href="/"]');
  ...
  test('Theme switching', async ({ page }) => {
    ...
    const themeSelector = page.locator('select').filter({ hasText: /auto|light|dark|Ç³É«|ÉîÉ«/ }).first();
